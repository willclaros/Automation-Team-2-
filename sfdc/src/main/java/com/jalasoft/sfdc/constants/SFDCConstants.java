package com.jalasoft.sfdc.constants;

/**
 * Constants class.
 *
 */
public final class SFDCConstants {

    public final static String CONTACT = "Contact";
    public final static String CONTACT_LAST_NAME = "LastName";
    public final static String CONTACT_FIRST_NAME = "FirstName";
    public final static String CONTACT_ACCOUNT_NAME = "Name";
    public final static String CONTACT_TITLE = "Title";
    public final static String CONTACT_EMAIL = "Email";
    public final static String CONTACT_PHONE = "Phone";
    public final static String CONTACT_OTHER_STREET = "OtherStreet";
    public final static String CONTACT_OTHER_CITY = "OtherCity";
    public final static String CONTACT_OTHER_STATE = "OtherState";
    public final static String CONTACT_OTHER_COUNTRY = "OtherCountry";

    public final static String QUOTE = "Quote";
    public final static String NAME_QUOTE = "Name";
    public final static String CREATE_DATE_QUOTE = "CreatedDate";
    public final static String CREATE_BY_ID_QUOTE = "CreatedById";
    public final static String LAST_MODIFIED_DATE_QUOTE = "LastModifiedDate";
    public final static String LAST_MODIFIED_BY_ID_QUOTE = "LastModifiedById";
    public final static String SYSTEM_MODSTAMP_QUOTE = "SystemModstamp";
    public final static String OPPORTUNITY_ID_QUOTE = "OpportunityId";
    public final static String PRICEBOOK_2ID_QUOTE = "Pricebook2Id";
    public final static String QUOTE_NUMBER_QUOTE = "QuoteNumber";
    public final static String SUBTOTAL_QUOTE = "Subtotal";
    public final static String TOTAL_PRICE_QUOTE = "TotalPrice";
    public final static String LINE_ITEM_COUNT_QUOTE = "LineItemCount";
    public final static String GRAND_TOTAL_QUOTE = "GrandTotal";

    public final static String PRICEBOOK = "Pricebook2";
    public final static String IS_DELETE_PRICEBOOK = "IsDeleted";
    public final static String NAME_PRICEBOOK = "Name";
    public final static String CREATE_DATE_PRICEBOOK = "CreateDate";
    public final static String CREATE_BY_ID_PRICEBOOK = "CreateById";
    public final static String LAST_MODIFIED_DATE_PRICEBOOK = "LastModifiedDate";
    public final static String IS_ACTIVE_PRICEBOOK = "IsActive";
    public final static String DESCRIPTION_PRICEBOOK = "Description";
    public final static String IS_STANDARD_PRICEBOOK = "IsStandard";

    public final static String ID = "id";
    public final static String TOTAL_SIZE = "totalSize";
    public final static String SOBJECTS = "sobjects";
    public final static String SLASH = "/";
}
